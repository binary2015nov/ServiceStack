	...
	public class MarkdownFormat : IViewEngine, IPlugin
	{
		...
		public void SerializeToStream(IRequest request, object response, Stream stream)
		{
			...
			if ((markdownPage = GetViewPageByResponse(dto, request)) == null)
			{
				...
				throw new InvalidDataException(ErrorPageNotFound.Fmt(GetPageName(dto, request)));
			}
		...
		public string RenderStaticPage(string filePath, bool renderHtml)
		{
			...
			if (!ContentPages.TryGetValue(filePath, out markdownPage))
				throw new InvalidDataException(ErrorPageNotFound.Fmt(filePath));
		...
		{
			if (markdownPage == null)
				throw new InvalidDataException(ErrorPageNotFound.Fmt(pageName));
