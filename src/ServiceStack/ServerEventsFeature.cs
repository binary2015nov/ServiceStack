...
namespace ServiceStack
{
	...
	*/
	...
	{
		private static readonly ILog Logger = LogManager.GetLogger(typeof(EventSubscription));
		...
		public void PublishRaw(string frame)
		{
			...
			{
				Logger.Warn("Could not publish notification to: " + frame.SafeSubstring(0, 50), ex);
				...
				{
					Logger.Error("OutputStream.Close()", innerEx);
				}
		...
		public void Dispose()
		{
			...
			{
				Logger.Error("Error ending subscription response", ex);
			}
	...
	{
		private static readonly ILog Logger = LogManager.GetLogger(typeof(MemoryServerEvents));
				...
				if (sub.HasChannel(channel))
				{
					if (now - sub.LastPulseAt > IdleTimeout || sub.IsClosed)
					{
						if (Logger.IsDebugEnabled)
							Logger.DebugFormat("[SSE-SERVER] Expired {0} Sub {1} on ({2})", selector, sub.SubscriptionId,
					...
					if (Logger.IsDebugEnabled)
						Logger.DebugFormat("[SSE-SERVER] Sending {0} msg to {1} on ({2})", selector, sub.SubscriptionId,
							string.Join(", ", sub.Channels));
		...
		public void Register(IEventSubscription subscription, Dictionary<string, string> connectArgs = null)
		{
			...
			{
				Logger.Error("Register: " + ex.Message, ex);
			...
			ConcurrentDictionary<string, ConcurrentDictionary<IEventSubscription, bool>> map)
		{
			...
			{
				Logger.Error("UnRegisterSubscription: " + ex.Message, ex);
				OnError?.Invoke(subscription, ex);
				throw;
			}
		}
